...
struct ForwardScrollRule: AutoPlayRule {
    ...
    func evaluate(basedOn situation: AutoPlayContainerSituation) -> AutoPlayRuleAction {
        ...
        let playableIndexPaths: Set<IndexPathWrapper> = Set(sortedItems.filter(\.canBePlayed).map { $0.indexPath })
        guard !playableIndexPaths.isEmpty else {
            return .startPlayItem(nil)
        }
        for item in sortedItems where item.indexPath.item % 2 == 0 {
            ...
            if itemTopInBounds < triggerLineLength {
                ...
                if playableIndexPaths.contains(targetIndexPath) {
...
struct LeftScrollRule: AutoPlayRule {
    ...
    func evaluate(basedOn situation: AutoPlayContainerSituation) -> AutoPlayRuleAction {
        ...
        let sortedItems = situation.visibleItems.sorted { $0.indexPath > $1.indexPath }
        for item in sortedItems where item.canBePlayed {
...
struct HorizontalCenterPlayRule: AutoPlayRule {
    ...
    func evaluate(basedOn situation: AutoPlayContainerSituation) -> AutoPlayRuleAction {
        let sortedItems = situation.visibleItems.filter(\.canBePlayed).sorted { $0.indexPath < $1.indexPath }
